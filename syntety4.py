...
insect_genome = read_bed_pandas("./训练集/anwengenome.bed",ncols=5)
nl_lnc = read_bed_pandas("./预测集/nllncRNA.bed",ncols=5)
insect_lnc = read_bed_pandas("./预测集/anwenlncRNA.bed",ncols=5)
#读入锚点文件
nl_insect_anchor = read_bed_pandas7("./nl_anwen_anchor.bed",ncols=7,
                                   columns = ['chrom1','start1','end1','chrom2','start2','end2','identity','extra1'])#1其他昆虫，2褐飞虱
#获取homolog文件
nl_insect_pos = read_tsv_pandas("./训练集/nl_anwen_final2.tsv")
mapping_file="./训练集/nl_anwen_final.tsv"
bed1_file="./训练集/nlgenome.bed"
bed2_file="./训练集/anwengenome.bed"
...
# # # 保存为 CSV 文件（无索引）
# df.to_csv("train_features_anchor_pos.csv", index=False)
...
# # 保存为 CSV 文件（无索引）
# df.to_csv("train_features_homolog_pos.csv", index=False)
...
# # 保存为 CSV 文件（无索引）
# df.to_csv("train_nl_insect_pos_feature.csv", index=False)
...
# #============================== 计算预测数据集和anchor及homolog相关的6个特征 ======================================================
# #两物种预测集的lncRNA对
# nl_insect_predict = pd.read_csv("./nl_anwen.csv",names=['nl_lnc','insect_lnc'])
# # # 获取两列的唯一值（假设需要所有唯一值组合）
# nl_values = nl_insect_predict['nl_lnc'].unique()  # 提取nl_lnc列的唯一值
# insect_values = nl_insect_predict['insect_lnc'].unique()  # 提取insect_lnc列的唯一值
import os
import csv

# 设置目标文件夹路径（可根据需要修改）
folder_path = './预测集/prediction/'
...
kindss=["anwen","bm","cngd","ls","mifeng","sf","yachong"]
for kinds in kindss:
    csv_path = './预测集/prediction/nl_'+ kinds+'_predict.csv'   


    result_df = pd.read_csv(csv_path)
    
    root_dir = './训练集/nl_'+ kinds
    insect_path = os.path.join(root_dir, f'{kinds}lncRNA.bed')
    insect_lnc = read_bed_pandas(insect_path,ncols=5)
    ...
    # # # 生成所有可能的配对组合
    # lncRNA_pairs = list(product(nl_values, insect_values))
    # # # 创建结果数据框
    # result_df = pd.DataFrame(lncRNA_pairs, columns=['id1', 'id2']) 
    # #计算预测数据集中lncRNA对和anchor相关的6个特征

    features_anchor=get_corr(nl_insect_anchor, nl_lnc,insect_lnc,result_df)
    ...
    anchor_path = f"{os.path.splitext(csv_path)[0]}_pred_features_anchor.csv"
    # 保存为 CSV 文件（无索引）
    ...
    # #homolog
    features_homolog =get_corr(nl_insect_homolog, nl_lnc,insect_lnc,result_df)
    ...
    homolog_path = f"{os.path.splitext(csv_path)[0]}_pred_features_homolog.csv"
    # 保存为 CSV 文件（无索引）
    df.to_csv(homolog_path, index=False)

    # #整合数据

    pred_nl_insect__feature = [a + b for a, b in zip(features_anchor, features_homolog)]

    # 将列表转换为 DataFrame，并添加列名
    df = pd.DataFrame(pred_nl_insect__feature, columns=[
        "nl_gene1", "insect_gene1",
        "mu1", "md1", "mf1",
        "proportionscoreu1", "proportionscored1", "proportionscoref1","nl_gene2", "insect_gene2",
        "mu2", "md2", "mf2",
    ...
    all_path = f"{os.path.splitext(csv_path)[0]}_pred_nl_insect_feature.csv"
    # 保存为 CSV 文件（无索引）
